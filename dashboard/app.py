...
from config import (
    DASH_PORT, DASHBOARD_REFRESH_INTERVAL_MS, LLM_REPORT_CACHE_TTL,
...
# ---------------------------------------------------------------------------
# LLM incident reporter (shares the API's Redis report cache)
# ---------------------------------------------------------------------------
...
def generate_report(decision: dict) -> dict:
    """Generate (or fetch cached) an LLM incident report for a decision dict."""
    ...
    result = _reporter.generate(decision)
    result["cached"] = False
    if cache_key:
        try:
            get_redis().setex(cache_key, LLM_REPORT_CACHE_TTL, json.dumps(result))
        except redis.RedisError:
            pass
    return result
...
app.layout = dbc.Container(
    ...
    children=[
        ...
        dbc.Row(dbc.Col(dbc.Card([
            html.Div(className="section-header d-flex justify-content-between align-items-center",
                     style={"color": "#00F5D4"}, children=[
                html.Span("🛡  AI Incident Report"),
                html.Button("⚡ Generate", id="report-btn", n_clicks=0,
                            className="report-btn"),
            ]),
            dbc.CardBody([
                html.Div("Latest flagged / blocked decision · generated by a local LLM",
                         id="report-meta", className="report-meta"),
                dcc.Loading(
                    type="dot", color="#00F5D4",
                    children=dcc.Markdown(
                        "Click **⚡ Generate** to produce a natural-language SOC "
                        "incident report for the most recent threat.",
                        id="report-output", className="report-body",
                    ),
                ),
            ], style={"padding": "14px 18px"}),
        ], className="chart-card")), className="mb-3 g-2"),
...
def on_generate_report(_):
    """Generate an LLM incident report for the most recent flag/block decision."""
    decisions = fetch_decisions(200)
    threat = next(
        (d for d in decisions if d.get("action") in ("flag", "block")), None
    )
    if threat is None:
        return (
            "No flagged or blocked decisions in the recent window yet. "
            "Run `make replay` to stream traffic, then try again.",
            "Awaiting threats…",
        )

    result = generate_report(threat)
    cls  = threat.get("predicted_class", "?").upper()
    conf = threat.get("confidence", 0.0)
    did  = threat.get("decision_id", "")[:8]

    if result.get("source") == "llm":
        engine = f"⚡ {result.get('model')} · {result.get('elapsed_ms', 0):.0f} ms"
    else:
        engine = "📝 template engine (Ollama offline)"
    if result.get("cached"):
        engine += " · cached"

    meta = f"Incident {cls} · confidence {conf:.0%} · id {did} · {engine}"
    return result.get("report", "_No report produced._"), meta
